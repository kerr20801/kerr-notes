...
```yaml
model_list:
  - model_name: ml/reasoning
    litellm_params:
      model: nvidia/gpt-oss-puzzle-88B
      api_base: http://[[PRIVATE_IP_d54cc2]]:8000/v1
```

App 只需要打 `http://localhost:4000/v1`，model name 用 `ml/reasoning`。換模型改 yaml 就好，四個 app 不動。
...
LiteLLM 是 1 對 1 路由。`ml/cloud1` → 一台機器。如果要比較，你得 app 自己呼叫 `ml/cloud1`、`ml/cloud2`……那 LiteLLM 就只是個 proxy，不是你真正需要的東西。
...
| 別名 | 角色 | 現在指向 |
|------|------|---------|
| `ml/reasoning` | 重推理 | DGX `nvidia/gpt-oss-puzzle-88B` |
| `ml/fast` | 快速生成 | edgexpert `gpt-oss-fast:latest` |
| `ml/stream` | 串流回答 | RTX `gemma4:latest` |
...
"always_serve_args": [
    ...
    "--served-model-name", "ml/reasoning"
]
```

換模型時，`--model` 改新的，`--served-model-name ml/reasoning` 保留不動。
...
OLLAMA_HOST=localhost:8000 ollama create ml/fast -f /tmp/m.modelfile
```

換模型時，`FROM` 改新 model，`ollama create ml/fast` 重跑。

---

## Cloud 模型的特殊問題

`llm-gemma4.teamml.work`（外部 API）只認識 `google/gemma-4-26B-A4B-it`，我們沒辦法在那邊建 alias。

解法：分開兩個概念。

```python
GEMMA4_CLOUD_MODEL     = "ml/cloud1"            # 邏輯別名，app 內部選擇用
...
{
  "ml/cloud1": {
    "name": "Gemma 4 31B (Teamml TPU)",
    "api_model": "google/gemma-4-26B-A4B-it",
    "url": "https://llm-gemma4.teamml.work/v1"
  }
}
```

未來加 `ml/cloud2`：在 config 多一條，比較迴圈自動涵蓋。這才是設計上真正該有的彈性。
